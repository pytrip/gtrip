...
class PyGtkTRiP:
	...
	if (self.checkbutton_ct.get_active()) and (self.ctx != False):
            ...
            #                                                          self.myslice)
            self.X,self.Y,self.cData = self.get_slice(self.ctx,self.myaxis,
                                                          self.myslice)            
    ...
    def update_labels(self):                
        ...
        if self.cdo >= 0:
            self.label_od_name.set_markup(str(self.do[self.cdo].patient_name))
            ...
            self.label_od_dimx.set_markup(str(self.do[self.cdo].dimx)) 
            self.label_od_dimy.set_markup(str(self.do[self.cdo].dimy)) 
            self.label_od_dimz.set_markup(str(self.do[self.cdo].dimz)) 
            ...
            self.label_od_byteo.set_markup(str(self.do[self.cdo].byte_order))
            self.label_od_dtype.set_markup(str(self.do[self.cdo].data_type))
	...
	filename = self.choose_file("*.ctx")
        ...
        if os.path.isfile(filename):
            ...
            while gtk.events_pending():
                gtk.main_iteration()
            self.ctx = CtxCube()
            self.ctx.read(filename)
            self.do.append(self.ctx) # New version
            self.cdo += 1
            #            self.hed = self.ctx.header
            self.hed = self.ctx
            self.ctx.min = self.ctx.cube.min()
            self.ctx.max = self.ctx.cube.max()
    ...
    def get_slice(self, obj, _idx, _slice):
        ...
        xmin = obj.xoffset+0.5
        ymin = obj.yoffset+0.5
        zmin = obj.zoffset+0.5
        xmax = xmin + obj.dimx
        ymax = ymin + obj.dimy
        zmax = zmin + obj.dimz
        ...
        x = arange(xmin,xmax,1) * obj.pixel_size
        y = arange(ymin,ymax,1) * obj.pixel_size
        z = (arange(zmin,zmax,1) * obj.slice_distance) + obj.slice_to_z(0)
        ...
        ymin *= obj.pixel_size
        ymax *= obj.pixel_size
        zmin = obj.slice_to_z(0) # position of first slice
        zmax = zmax * obj.slice_distance + zmin
